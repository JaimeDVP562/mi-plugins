...
class AutomatorWP_Drip_Remove_Subscriber extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Remove subscriber', 'automatorwp' ),
            'select_option'     => __( '<strong>Remove subscriber</strong>', 'automatorwp' ),
            'edit_label'        => sprintf( __( 'Remove subscriber', 'automatorwp' ) ),
            'log_label'         => sprintf( __( 'Remove subscriber', 'automatorwp' ) ),
            'options'           => array(
                // No options
            ),
        ) );
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_drip_remove_subscriber( $user->user_email );

        if ( $response === 203 || $response === 204 ) {

            $this->result = sprintf( __( 'Subscriber %s removed from Drip', 'automatorwp' ), $user->user_email );

        } else $this->result = sprintf( __( 'Error removing subscriber %s: ' . $response, 'automatorwp' ), $user->user_email );
