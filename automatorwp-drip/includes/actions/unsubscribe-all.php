<?php
/**
 * Unsubscribe Subscriber From All Email Marketing
 *
 * @package     AutomatorWP\Integrations\Drip\Actions\Unsubscribe_All
...
class AutomatorWP_Drip_Unsubscribe_All extends AutomatorWP_Integration_Action {

    public $integration = 'drip';
    public $action      = 'drip_unsubscribe_all';
    public $result      = '';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'         => __( 'Unsubscribe subscriber from all email marketing', 'automatorwp-drip' ),
            'select_option' => __( 'Unsubscribe <strong>subscriber</strong> from all email marketing', 'automatorwp-drip' ),
            /* translators: %1$s: email. */
            'edit_label'    => sprintf( __( 'Unsubscribe %1$s from all email marketing', 'automatorwp-drip' ), '{email}' ),
            /* translators: %1$s: email. */
            'log_label'     => sprintf( __( 'Unsubscribe %1$s from all email marketing', 'automatorwp-drip' ), '{email}' ),
            'options'       => array(
                'email' => array(
                    'from'    => 'email',
                    'default' => __( 'email', 'automatorwp-drip' ),
                    'fields'  => array(
                        'email' => array(
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_drip_unsubscribe_all( $email );

        if ( $response['code'] === 204 || $response['code'] === 200 ) {
            $this->result = sprintf( __( '%s unsubscribed from all email marketing.', 'automatorwp-drip' ), $email );
        } else {
...
new AutomatorWP_Drip_Unsubscribe_All();
