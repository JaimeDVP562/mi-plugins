<?php
/**
 * Subscription Expired Category
 *
 * @package     AutomatorWP\Integrations\WooCommerce\Triggers\Subscription_Expired_Category
...
class AutomatorWP_WooCommerce_Subscription_Expired_Category extends AutomatorWP_Integration_Trigger {

    public $integration = 'woocommerce';
    public $trigger = 'woocommerce_subscription_expired_category';
    ...
    public function register() {
        ...
        automatorwp_register_trigger( $this->trigger, array(
            ...
            'label'             => __( 'User subscription of a category expires', 'automatorwp-woocommerce' ),
            'select_option'     => __( 'User subscription of <strong>a category</strong> expires', 'automatorwp-woocommerce' ),
            /* translators: %1$s: Term title. %2$s: Number of times. */
            'edit_label'        => sprintf( __( 'User subscription of %1$s expires %2$s time(s)', 'automatorwp-woocommerce' ), '{term}', '{times}' ),
            /* translators: %1$s: Term title. */
            'log_label'         => sprintf( __( 'User subscription of %1$s expires', 'automatorwp-woocommerce' ), '{term}' ),
    ...
    public function listener( $subscription ) {
        ...
        if( ! $subscription->has_status( array( 'expired' ) ) ) {
            return;
        }
...
new AutomatorWP_WooCommerce_Subscription_Expired_Category();