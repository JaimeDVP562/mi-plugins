(function( $ ) {
    ...
    function populateBackingFromCMB2(option_form){try{if(!option_form||!option_form.length)return;var $=window.jQuery||jQuery;var mapping={email:['[data-option="email"]','input[type="email"]','input[name="email"]'],first_name:['[data-option="first_name"]','input[name*="first_name"]','input[id*="first_name-"]'],last_name:['[data-option="last_name"]','input[name*="last_name"]','input[id*="last_name-"]'],addressbook_id:['select[data-option="addressbook_id"]','select[name*="addressbook_id"]','input[name*="addressbook_id"]']};Object.keys(mapping).forEach(function(name){try{var selectors=mapping[name];var val='';for(var i=0;i<selectors.length;i++){try{var src=option_form.find(selectors[i]).first();if(src&&src.length&&src.val()){val=src.val();break;}}catch(e){} }if(!val){try{var item=option_form.closest('.automatorwp-automation-item');if(item&&item.length){var alt=item.find(selectors.join(',' )).first();if(alt&&alt.length&&alt.val())val=alt.val();}}catch(e){} }if(val){try{option_form.find('input[type="hidden"][name="'+name+'"]').each(function(){try{$(this).val(val);}catch(ignore){}});}catch(e){}try{$('body').find('input[type="hidden"][name="'+name+'"]').each(function(){try{$(this).val(val);}catch(ignore){}});}catch(e){}if(name==='email'){try{var vf=option_form.find('input[data-automatorwp-fallback="1"]').first();if(vf&&vf.length&&!vf.val())vf.val(val);}catch(e){}}} }catch(e){} });}catch(e){} }
    ...
    // Ensure backing hidden inputs exist for important fields so AutomatorWP saves them.
    function ensureBackingFields(option_form) {
        try {
            if (!option_form || !option_form.length) return;
            var $ = window.jQuery || jQuery;
            var visibleEmail = option_form.find('input[type="email"]').first();
            if (!visibleEmail || !visibleEmail.length) visibleEmail = option_form.find('input[name="email"]').first();
            var visibleFirst = option_form.find('input[name*="first_name"]').first();
            var visibleLast = option_form.find('input[name*="last_name"]').first();
            ...
            function ensureHiddenInContainers(name) {
                ...
                try {
                    ...
                    hid = $('<input type="hidden" />').attr('name', name).attr('data-automatorwp-backing','1');
                    try { option_form.append(hid); } catch(e) { try { if (item && item.length) item.append(hid); else $('body').append(hid); } catch(e){ $('body').append(hid); } }
                } catch(e) { try { hid = $('<input type="hidden" />').attr('name', name).attr('data-automatorwp-backing','1'); $('body').append(hid); } catch(ignore) { hid = null; } }
                return hid;
            }
            ...
            try { if (visibleEmail && visibleEmail.length) hidEmail.val(visibleEmail.val()); } catch(e){}
            try { if (visibleFirst && visibleFirst.length) hidFirst.val(visibleFirst.val()); } catch(e){}
            try { if (visibleLast && visibleLast.length) hidLast.val(visibleLast.val()); } catch(e){}
            try { if (addressbookSelect && addressbookSelect.length) hidAddressbook.val(addressbookSelect.val()); } catch(e){}
            ...
            try { if (visibleEmail && visibleEmail.length) visibleEmail.off('input.automatorwp_backing').on('input.automatorwp_backing', function(){ syncBacking('email', $(this).val()); }); if (visibleFirst && visibleFirst.length) visibleFirst.off('input.automatorwp_backing').on('input.automatorwp_backing', function(){ syncBacking('first_name', $(this).val()); }); if (visibleLast && visibleLast.length) visibleLast.off('input.automatorwp_backing').on('input.automatorwp_backing', function(){ syncBacking('last_name', $(this).val()); }); if (addressbookSelect && addressbookSelect.length) addressbookSelect.off('change.automatorwp_backing').on('change.automatorwp_backing', function(){ syncBacking('addressbook_id', $(this).val()); }); } catch(e){}

            try { option_form.find('.automatorwp-sendpulse-email-select').each(function(){ var s = $(this); s.off('change.automatorwp_backing_prog').on('change.automatorwp_backing_prog', function(){ syncBacking('email', $(this).val()); }); }); } catch(e){}
    ...
    function initAddressbookSelector(option_form, opts) {
                    ...
                    if ( !loadBtn.length ) {
                        ...
                        loadBtn.on('click', function(){
                            ...
                            $.post( _getAjaxUrl(), { action: 'automatorwp_sendpulse_list_subscribers', nonce: automatorwp_sendpulse.nonce, addressbook_id: ab, page: 1, per_page: 100 }, function(resp){
                                ...
                                if (!resp || resp.success !== true) { alert('Failed to load subscribers: ' + (resp && resp.data && resp.data.message ? resp.data.message : 'unknown')); return; }
                                ...
                                data.forEach(function(e){ var em = e.email || e.email_address || ''; if (!em) return; sel.append($('<option>').attr('value', em).text(em + ' (' + (e.status_explain || e.status || '') + ')')); });
                                emailWrapper.append(sel);
                                sel.on('change', function(){ var v = $(this).val(); var emailInput = option_form.find('input[type="email"]').first(); if (emailInput && emailInput.length) { emailInput.val(v); } else { var hidden = option_form.find('input[name="email"]'); if (!hidden.length) { hidden = $('<input type="hidden" name="email">'); emailWrapper.after(hidden); } hidden.val(v); } });
        ...
        if ( !loadBtn.length ) {
            ...
            loadBtn.on('click', function(){
                ...
                $.post( _getAjaxUrl(), { action: 'automatorwp_sendpulse_list_subscribers', nonce: automatorwp_sendpulse.nonce, addressbook_id: ab, page: 1, per_page: 100 }, function(resp){
                    ...
                    if (!emailWrapper.length) {
                        var emailInput = option_form.find('input[type="email"]').first();
                        if (typeof loadBtn !== 'undefined' && loadBtn && loadBtn.length) { emailWrapper = $('<div class="automatorwp-sendpulse-email-wrapper" style="margin-top:6px;"></div>'); loadBtn.after(emailWrapper); } else if (emailInput && emailInput.length) { emailWrapper = $('<div class="automatorwp-sendpulse-email-wrapper" style="margin-top:6px;"></div>'); emailInput.after(emailWrapper); } else { emailWrapper = $('<div class="automatorwp-sendpulse-email-wrapper" style="margin-top:6px;"></div>'); row_for_sel.after(emailWrapper); }
                    } else { emailWrapper.empty(); }
                    var sel = $('<select class="automatorwp-sendpulse-email-select" style="width:100%;"></select>');
                    sel.append($('<option>').attr('value','').text('-- Select email --'));
                    data.forEach(function(e){ var em = e.email || e.email_address || ''; if (!em) return; sel.append($('<option>').attr('value', em).text(em + ' (' + (e.status_explain || e.status || '') + ')')); });
        ...
        try { select.off('change.automatorwp_sendpulse_sync').on('change.automatorwp_sendpulse_sync', function(){ var v = $(this).val(); if (input && input.length && !input.is('select')) input.val(v); if (v) loadBtn.show(); else loadBtn.hide(); }); } catch(e){}

        if ( autoLoad && select.val() ) { try { loadBtn.trigger('click'); } catch(e){} }
