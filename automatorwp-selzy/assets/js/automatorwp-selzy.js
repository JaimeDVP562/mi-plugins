(function( $ ) {
    ...
    var prefix = 'automatorwp-selzy-';
    var _prefix = 'automatorwp_selzy_';
    ...
    $('body').on('click', '.automatorwp_settings #' + _prefix + 'authorize', function(e) {
        ...
        if( token.length === 0 ) {
            ...
            response_wrap.html( 'API token is required to connect with Selzy' );
        ...
        $.post(
            ajaxurl,
            {
                action: 'automatorwp_selzy_authorize',
                nonce: automatorwp_selzy.nonce,
                token: token,
            },
            function( response ) {
