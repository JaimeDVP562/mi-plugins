<?php
/**
 * Delete Subscriber To List
 *
 * @package     AutomatorWP\Integrations\Selzy\Actions\Subscriber_Delete_from_List
...
class AutomatorWP_Selzy_Del_Subscriber extends AutomatorWP_Integration_Action {

    public $integration = 'selzy';
    public $action = 'selzy_del_subscriber';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Delete subscriber from list', 'automatorwp-selzy' ),
            'select_option'     => __( 'Delete <strong>subscriber</strong> from <strong>list</strong>', 'automatorwp-selzy' ),
            /* translators: %1$s: List. */
            'edit_label'        => sprintf( __( 'Delete %1$s from %2$s', 'automatorwp-selzy' ), '{subscriber}', '{list}' ),
            /* translators: %1$s: List. */
            'log_label'         => sprintf( __( 'Delete %1$s from %2$s', 'automatorwp-selzy' ), '{subscriber}', '{list}' ),
            'options'           => array(
                'subscriber' => array(
                    'from' => 'email',
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_selzy_del_contact( $subscriber_email, $list_id );


        if ( $response === 200 ) {
            $this->result = __( 'Deleted subscriber from list', 'automatorwp-selzy' );
        } else {
            $this->result = __( 'Couldnt delete subscriber from list', 'automatorwp-selzy' );
        }
...
new AutomatorWP_Selzy_Del_Subscriber();