(function( $ ) {
    ...
    var prefix = 'automatorwp-bento-';
    var _prefix = 'automatorwp_bento_';
    ...
    $('body').on('click', '.automatorwp_settings #' + _prefix + 'authorize', function(e) {
        ...
        var key = $('#' + _prefix + 'client_id').val();
        var secret = $('#' + _prefix + 'client_secret').val();
        ...
        if( key.length === 0 || secret.length === 0 ) {
            ...
            response_wrap.html( 'All fields are required to connect with Bento' );
        ...
        $.post(
            ajaxurl,
            {
                action: 'automatorwp_bento_authorize',
                nonce: automatorwp_bento.nonce,
                key: key,
                secret: secret,
            },
            function( response ) {
