<?php
/**
 * Action: Remove Member from Google Group
 *
...
class AutomatorWP_GoogleGroups_Remove_Member extends AutomatorWP_Integration_Action {

    public $integration = 'googlegroups';
    public $action      = 'googlegroups_remove_member';

    public function register() {

        automatorwp_register_action( $this->action, array(
            'integration'   => $this->integration,
            'label'         => __( 'Remove a member from a Google Group', 'automatorwp-googlegroups' ),
            'select_option' => __( 'Remove a <strong>member</strong> from a Google Group', 'automatorwp-googlegroups' ),
            'edit_label'    => sprintf( __( 'Remove member %1$s from group %2$s', 'automatorwp-googlegroups' ), '{member_email}', '{group}' ),
            'log_label'     => sprintf( __( 'Removed member %1$s from group %2$s', 'automatorwp-googlegroups' ), '{member_email}', '{group}' ),
            'options'       => array(
                // group selector at top level (same as add/update)
                ...
                // member selector (ajax) filtered by selected group
                'member_email' => automatorwp_utilities_ajax_selector_option( array(
                    'field'          => 'member_email',
                    'name'           => __( 'Member', 'automatorwp-googlegroups' ),
                    'option_none'    => false,
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = $service ? $service->remove_member( $group, $email_final ) : 0;

        if ( in_array( $response, array( 200 ), true ) ) {

            do_action( 'automatorwp_googlegroups_member_removed', $user_id, array( 'group' => $group, 'member' => $email_final ) );
            $this->result = __( 'Member removed from Google Group successfully.', 'automatorwp-googlegroups' );

        } elseif ( $response === 404 ) {

            // Member not found - treat as success by default
            $this->result = __( 'Member not found in Google Group (treated as success).', 'automatorwp-googlegroups' );

        } else {
            $this->result = __( 'Failed to remove member from Google Group.', 'automatorwp-googlegroups' );
        }
...
new AutomatorWP_GoogleGroups_Remove_Member();
