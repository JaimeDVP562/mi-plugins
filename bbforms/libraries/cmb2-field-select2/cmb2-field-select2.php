...
if( !class_exists( 'RGC_CMB2_Field_Select2' ) ) {
    ...
    class RGC_CMB2_Field_Select2 {
        ...
        public function get_results() {
            ...
            $q = isset( $_REQUEST['q'] ) ? $wpdb->esc_like( wp_unslash( $_REQUEST['q'] ) ) : '';
            ...
            $query_args = array(
                ...
                'post_type_not_in' => isset( $_REQUEST['post_type_not_in'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['post_type_not_in'] ) ) : '',
                'post_status' => isset( $_REQUEST['post_status'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['post_status'] ) ) : '',
                'post_status_not_in' => isset( $_REQUEST['post_status_not_in'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['post_status_not_in'] ) ) : '',
                // from = users
                // Nothing
                // from = terms
                ...
                'id_field' => isset( $_REQUEST['id_field'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['id_field'] ) ) : '',
                'text_field' => isset( $_REQUEST['text_field'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['text_field'] ) ) : '',
                'label_field' => isset( $_REQUEST['label_field'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['label_field'] ) ) : '',
            );
            ...
            // Query results
            $results = $wpdb->get_results( $wpdb->prepare( $sql . " LIMIT %d, %d", $offset, $limit ) );
