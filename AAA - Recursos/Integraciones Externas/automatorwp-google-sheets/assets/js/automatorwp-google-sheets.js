(function( $ ) {
    ...
    var prefix = 'automatorwp-google-sheets-';
    var _prefix = 'automatorwp_google_sheets_';
    ...
    $('body').on('click', '.automatorwp_settings #' + _prefix + 'authorize', function(e) {
        ...
        if( client_id.length === 0 || client_secret.length === 0 ) {
            ...
            response_wrap.html( 'All fields are required to connect with Google Sheets' );
        ...
        $.post(
            ajaxurl,
            {
                action: 'automatorwp_google_sheets_authorize',
                nonce: automatorwp_google_sheets.nonce,
                client_id: client_id,
    ...
    $('body').on('change', '.automatorwp-action-google-sheets-add-row .cmb2-id-spreadsheet select, '
