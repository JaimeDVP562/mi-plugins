(function($){var prefix='automatorwp-senpulse-';var _prefix='automatorwp_senpulse_';$('body').on('click','.automatorwp_settings #'+_prefix+'authorize',function(e){e.preventDefault();var button=$(this);var wrapper=button.parent();var token=$('#'+_prefix+'token').val();var response_wrap=wrapper.find('#'+_prefix+'response');if(!response_wrap.length){wrapper.append('<div id="'+_prefix+'response" style="display: none; margin-top: 10px;"></div>');response_wrap=wrapper.find('#'+_prefix+'response')}
if(token.length===0){response_wrap.addClass('automatorwp-notice-error');response_wrap.html('API token is required to connect with senpulse');response_wrap.slideDown('fast');return}
response_wrap.slideUp('fast');response_wrap.attr('class','');wrapper.append('<span class="spinner is-active" style="float: none;"></span>');button.prop('disabled',!0);$.post(ajaxurl,{action:'automatorwp_senpulse_authorize',nonce:automatorwp_senpulse.nonce,token:token,},function(response){response_wrap.addClass('automatorwp-notice-'+(response.success===!0?'success':'error'));response_wrap.html((response.data.message!==undefined?response.data.message:response.data));response_wrap.slideDown('fast');wrapper.find('.spinner').remove();if(response.success===!0&&response.data.redirect_url!==undefined){window.location=response.data.redirect_url;return}
button.prop('disabled',!1)})});$('body').on('change','.automatorwp-action-senpulse-create-list .cmb2-id-team select, '+'.automatorwp-action-senpulse-add-comment-task .cmb2-id-team select, '+'.automatorwp-action-senpulse-add-tag-task .cmb2-id-team select, '+'.automatorwp-action-senpulse-create-task .cmb2-id-team select, '+'.automatorwp-action-senpulse-remove-task .cmb2-id-team select, '+'.automatorwp-action-senpulse-remove-tag-task .cmb2-id-team select',function(e,first_change){var team=$(this).closest('.cmb-row');var space_team=team.next('.cmb2-id-space');var team_id=$(this).val();if(first_change===undefined){first_change=!1}
...
space_selector.removeAttr('data-select2-id');automatorwp_ajax_selector(space_selector);if(first_change){space_team.show()}else{space_team.slideDown('fast')}}});$('body').on('change','.automatorwp-action-senpulse-create-list .cmb2-id-space select, '+'.automatorwp-action-senpulse-add-comment-task .cmb2-id-space select, '+'.automatorwp-action-senpulse-add-tag-task .cmb2-id-space select, '+'.automatorwp-action-senpulse-create-task .cmb2-id-space select, '+'.automatorwp-action-senpulse-remove-task .cmb2-id-space select, '+'.automatorwp-action-senpulse-remove-tag-task .cmb2-id-space select',function(e,first_change){var space=$(this).closest('.cmb-row');var folder_space=space.next('.cmb2-id-folder');var space_id=$(this).val();if(first_change===undefined){first_change=!1}
