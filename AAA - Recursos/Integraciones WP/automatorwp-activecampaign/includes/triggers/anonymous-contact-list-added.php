<?php
/**
 * Anonymous Contact List Added
 *
 * @package     AutomatorWP\Integrations\ActiveCampaign\Triggers\Anonymous_Contact_List_Added
...
class AutomatorWP_ActiveCampaign_Anonymous_Contact_List_Added extends AutomatorWP_Integration_Trigger {

    public $integration = 'activecampaign';
    public $trigger = 'activecampaign_anonymous_contact_list_added';
    ...
    public function register() {
        ...
        automatorwp_register_trigger( $this->trigger, array(
            ...
            'anonymous'         => true,
            'label'             => __( 'Contact added to list', 'automatorwp-activecampaign' ),
            'select_option'     => __( 'Contact added to <strong>list</strong>', 'automatorwp-activecampaign' ),
            /* translators: %1$s: List. %2$s: Number of times. */
            'edit_label'        => sprintf( __( 'Contact added to %1$s %2$s time(s)', 'automatorwp-activecampaign' ), '{list}', '{times}' ),
            /* translators: %1$s: List. */
            'log_label'         => sprintf( __( 'Contact added to %1$s', 'automatorwp-activecampaign' ), '{list}' ),
            'action'            => 'automatorwp_activecampaign_contact_list_added',
            ...
            'options'           => array(
                'list' => automatorwp_utilities_ajax_selector_option( array(
                    ...
                    'option_none_label' => __( 'Any list', 'automatorwp-activecampaign' ),
            ...
            'tags' => array_merge(
                automatorwp_activecampaign_get_webhook_lists()
            )
        ) );
    ...
    public function listener( $params ) {
        ...
        $list = $params['list'];
        $list_name = automatorwp_activecampaign_get_list_name( $list );

		/* translators: %1$s: Email. %2$s: List Name .*/
		$this->result = sprintf( __( '%1$s was added to %2$s', 'automatorwp-activecampaign' ), $email, $list_name );
    ...
    public function anonymous_deserves_trigger( $deserves_trigger, $trigger, $event, $trigger_options, $automation ) {
        ...
        if( $trigger_options['list'] !== 'any' && absint( $event['list'] ) !== absint( $trigger_options['list'] ) ) {
            return false;
        }
    ...
    public function log_meta( $log_meta, $trigger, $event, $trigger_options, $automation ) {
        ...
        $log_meta['list'] = ( isset( $event['list_name'] ) ? $event['list_name'] : '' );
    ...
    public function log_fields( $log_fields, $log, $object ) {
        ...
        $log_fields['list'] = array(
            'name' => __( 'List added:', 'automatorwp-activecampaign' ),
            'type' => 'text',
        );
...
new AutomatorWP_ActiveCampaign_Anonymous_Contact_List_Added();