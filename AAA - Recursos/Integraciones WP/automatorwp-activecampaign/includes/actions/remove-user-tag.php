 ...
 *
 * @package     AutomatorWP\Integrations\ActiveCampaign\Actions\Remove_User_Tag
...
class AutomatorWP_ActiveCampaign_Remove_User_Tag extends AutomatorWP_Integration_Action {

    public $integration = 'activecampaign';
    public $action = 'activecampaign_remove_user_tag';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Remove tag from user', 'automatorwp-activecampaign' ),
            'select_option'     => __( 'Remove <strong>tag</strong> from <strong>user</strong>', 'automatorwp-activecampaign' ),
            /* translators: %1$s: Tag. */
            'edit_label'        => sprintf( __( 'Remove %1$s from user', 'automatorwp-activecampaign' ), '{tag}' ),
            /* translators: %1$s: Tag. */
            'log_label'         => sprintf( __( 'Remove %1$s from user', 'automatorwp-activecampaign' ), '{tag}' ),
            'options'           => array(
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        // To get all tags from the user
        $response = automatorwp_activecampaign_get_contact_tags( $contact_id );

        // Bail if user doesn't have any tag associated
        if (empty($response['contactTags'])){

            $this->result = sprintf( __( '%s does not have any tag associated', 'automatorwp-activecampaign' ), $user->user_email );
            return;
        ...
        if ( !isset ( $tag_delete )){
            $this->result = sprintf( __( 'Tag is not associated to %s', 'automatorwp-activecampaign' ), $user->user_email );
            return;
        }

        automatorwp_activecampaign_delete_contact_tags( $tag_delete );
                    
        $this->result = sprintf( __( 'Tag has been deleted from %s', 'automatorwp-activecampaign' ), $user->user_email );
    }
...
new AutomatorWP_ActiveCampaign_Remove_User_Tag();