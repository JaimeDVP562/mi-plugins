<?php
/**
 * Create and assign tag to user
 *
 * @package     AutomatorWP\Integrations\ActiveCampaign\Actions\Create_User_Tag
...
class AutomatorWP_ActiveCampaign_Create_User_Tag extends AutomatorWP_Integration_Action {

    public $integration = 'activecampaign';
    public $action = 'activecampaign_create_user_tag';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Create tag and assign to user', 'automatorwp-activecampaign' ),
            'select_option'     => __( 'Create <strong>tag</strong> and assign to <strong>user</strong>', 'automatorwp-activecampaign' ),
            /* translators: %1$s: Tag. */
            'edit_label'        => sprintf( __( 'Create %1$s and assign to user', 'automatorwp-activecampaign' ), '{tag}' ),
            /* translators: %1$s: Tag. */
            'log_label'         => sprintf( __( 'Create %1$s and assign to user', 'automatorwp-activecampaign' ), '{tag}' ),
            'options'           => array(
                'tag' => array(
                    'from' => 'tag',
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        if ( $tag_id === false ) {
            ...
            $this->result = sprintf( __( 'Tag %s created and assigned to %s', 'automatorwp-activecampaign' ), $tag_name, $user->user_email );
        } else {
            $this->result = sprintf( __( 'Tag %s found and assigned to %s', 'automatorwp-activecampaign' ), $tag_name, $user->user_email );
        }
...
new AutomatorWP_ActiveCampaign_Create_User_Tag();