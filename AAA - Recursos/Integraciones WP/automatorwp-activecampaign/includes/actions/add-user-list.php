 ...
 * Add user list
 *
 * @package     AutomatorWP\Integrations\ActiveCampaign\Actions\Add_User_List
...
class AutomatorWP_ActiveCampaign_Add_User_List extends AutomatorWP_Integration_Action {

    public $integration = 'activecampaign';
    public $action = 'activecampaign_add_user_list';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add user to list', 'automatorwp-activecampaign' ),
            'select_option'     => __( 'Add <strong>user</strong> to <strong>list</strong>', 'automatorwp-activecampaign' ),
            /* translators: %1$s: List name. */
            'edit_label'        => sprintf( __( 'Add user to %1$s', 'automatorwp-activecampaign' ), '{list}' ),
            /* translators: %1$s: List name. */
            'log_label'         => sprintf( __( 'Add user to %1$s', 'automatorwp-activecampaign' ), '{list}' ),
            'options'           => array(
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        if ( ! empty( $response['contactLists'] ) ){
            ...
            foreach ( $response['contactLists'] as $contact_list ){
                ...
                if ( $list_id == $contact_list['list'] && $contact_list['status'] == '1'){
                    $this->result = sprintf( __( '%s is already subscribed to that list', 'automatorwp-activecampaign' ), $user->user_email );
                    return;
                }
            }
        ...
        $this->result = sprintf( __( '%s added to list', 'automatorwp-activecampaign' ), $user->user_email );
...
new AutomatorWP_ActiveCampaign_Add_User_List();