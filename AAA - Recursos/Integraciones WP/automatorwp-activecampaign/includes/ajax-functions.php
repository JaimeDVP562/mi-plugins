...
function automatorwp_activecampaign_ajax_authorize() {
    ...
    if( ! current_user_can( automatorwp_get_manager_capability() ) ) {
        wp_send_json_error( __( 'You\'re not allowed to perform this action.', 'automatorwp-activecampaign' ) );
    }
    ...
    if( empty( $url ) || empty( $key ) ) {
        wp_send_json_error( array( 'message' => __( 'All fields are required to connect with ActiveCampaign', 'automatorwp-activecampaign' ) ) );
        return;
    }
    ...
    if ( isset( $response->errors ) ){
        wp_send_json_error (array( 'message' => __( 'Please, check your credentials', 'automatorwp-activecampaign' ) ) );
        return;
    }
    ...
    wp_send_json_success( array(
        'message' => __( 'Correct data to connect with ActiveCampaign', 'automatorwp-activecampaign' ),
...
function automatorwp_activecampaign_ajax_refresh( ) {
    ...
    wp_send_json_success( array(
        'message' => __( 'Webhook URL refreshed', 'automatorwp-activecampaign' ),
        'redirect_url' => $admin_url
    ) );
 ...
 * @since 1.0.0
 */
function automatorwp_activecampaign_ajax_get_lists() {
    // Security check, forces to die if not security passed
    check_ajax_referer( 'automatorwp_admin', 'nonce' );
    ...
    $lists = automatorwp_activecampaign_get_lists( $search, $page );
...
add_action( 'wp_ajax_automatorwp_activecampaign_get_lists', 'automatorwp_activecampaign_ajax_get_lists' );