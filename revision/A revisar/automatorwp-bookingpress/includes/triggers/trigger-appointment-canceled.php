...
/**
 * Trigger AutomatorWP when an appointment is canceled for a real WordPress user
 */
if (!defined('ABSPATH')) exit;

class AutomatorWP_BookingPress_Appointment_Canceled extends AutomatorWP_Integration_Trigger
{

    public $integration = 'bookingpress';
    public $trigger = 'bookingpress_appointment_canceled';

    public function register()
    {
        automatorwp_register_trigger($this->trigger, array(
            'integration'   => $this->integration,
            'label'         => __('User cancels an appointment', 'automatorwp-bookingpress'),
            'select_option' => __('User <strong>cancels</strong> an appointment', 'automatorwp-bookingpress'),
            'edit_label'    => sprintf(__('User cancels an appointment %1$s time(s)', 'automatorwp-bookingpress'), '{times}'),
            'log_label'     => __('User cancels an appointment', 'automatorwp-bookingpress'),
    ...
    {
        if ((string) $appointment_status !== '3') return; // 3 = Canceled
...
new AutomatorWP_BookingPress_Appointment_Canceled();
