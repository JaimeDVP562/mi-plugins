...
class AutomatorWP_BookingPress_Appointment_Pending extends AutomatorWP_Integration_Trigger
{

    public $integration = 'bookingpress';
    public $trigger = 'bookingpress_appointment_pending';

    public function register()
    {
        automatorwp_register_trigger($this->trigger, array(
            'integration'   => $this->integration,
            'label'         => __('User appointment is set to pending', 'automatorwp-bookingpress'),
            'select_option' => __('User appointment is set to <strong>pending</strong>', 'automatorwp-bookingpress'),
            'edit_label'    => sprintf(__('User appointment is set to pending %1$s time(s)', 'automatorwp-bookingpress'), '{times}'),
            'log_label'     => __('User appointment is set to pending', 'automatorwp-bookingpress'),
    ...
    /**
     * Trigger AutomatorWP when an appointment is pending for a real WordPress user
     */
    public function listener($appointment_id, $appointment_status)
    {
        if ((string) $appointment_status !== '2') return; // 2 = Pending
...
new AutomatorWP_BookingPress_Appointment_Pending();
