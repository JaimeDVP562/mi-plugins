 ...
 *
 * @package     AutomatorWP\Integrations\MailMint\Actions\Remove_Tag
...
class AutomatorWP_MailMint_Remove_Tag extends AutomatorWP_Integration_Action
{
    public $integration = 'mailmint';
    public $action      = 'mailmint_remove_tag';
    public $result      = '';
    ...
    public function register()
    {
        automatorwp_register_action( $this->action, array(
            ...
            'label'         => __( 'Remove a tag from a Mail Mint contact', 'automatorwp-mailmint' ),
            'select_option' => __( 'Remove a <strong>tag</strong> from a Mail Mint contact', 'automatorwp-mailmint' ),
            /* translators: %1$s: Tag. */
            'edit_label'    => sprintf( __( 'Remove tag %1$s from Mail Mint contact', 'automatorwp-mailmint' ), '{tag}' ),
            /* translators: %1$s: Tag. */
            'log_label'     => sprintf( __( 'Remove tag %1$s from Mail Mint contact', 'automatorwp-mailmint' ), '{tag}' ),
            'options'       => array(
                'tag' => array(
                    'from'    => 'tag',
                    ...
                    'fields'  => array(
                        ...
                        'tag' => automatorwp_utilities_ajax_selector_field( array(
                            ...
                            'desc'       => __( 'Select the Mail Mint tag to remove from the contact.', 'automatorwp-mailmint' ),
                            'type'       => 'select',
                            'field'      => 'tag',
    ...
    public function execute( $action, $user_id, $action_options, $automation )
    {
        ...
        automatorwp_mailmint_remove_groups_from_contact( (int) $contact->id, array( $tag_id ) );

        $tag  = automatorwp_mailmint_get_group( $tag_id );
        $name = $tag ? $tag->title : $tag_id;

        $this->result = sprintf( __( 'Tag "%1$s" removed from %2$s.', 'automatorwp-mailmint' ), $name, $email );
    }
...
new AutomatorWP_MailMint_Remove_Tag();
