 ...
 *
 * @package     AutomatorWP\Integrations\MailMint\Actions\Remove_From_List
...
class AutomatorWP_MailMint_Remove_From_List extends AutomatorWP_Integration_Action
{
    public $integration = 'mailmint';
    public $action      = 'mailmint_remove_from_list';
    public $result      = '';
    ...
    public function register()
    {
        automatorwp_register_action( $this->action, array(
            ...
            'label'         => __( 'Remove a Mail Mint contact from a list', 'automatorwp-mailmint' ),
            'select_option' => __( 'Remove a Mail Mint contact from a <strong>list</strong>', 'automatorwp-mailmint' ),
            /* translators: %1$s: List. */
            'edit_label'    => sprintf( __( 'Remove Mail Mint contact from list %1$s', 'automatorwp-mailmint' ), '{list}' ),
            /* translators: %1$s: List. */
            'log_label'     => sprintf( __( 'Remove Mail Mint contact from list %1$s', 'automatorwp-mailmint' ), '{list}' ),
            'options'       => array(
                'list' => array(
                    'from'    => 'list',
                    ...
                    'fields'  => array(
                        ...
                        'list' => automatorwp_utilities_ajax_selector_field( array(
                            ...
                            'desc'       => __( 'Select the Mail Mint list to remove the contact from.', 'automatorwp-mailmint' ),
                            'type'       => 'select',
                            'field'      => 'list',
    ...
    public function execute( $action, $user_id, $action_options, $automation )
    {
        ...
        automatorwp_mailmint_remove_groups_from_contact( (int) $contact->id, array( $list_id ) );

        $list = automatorwp_mailmint_get_group( $list_id );
        $name = $list ? $list->title : $list_id;

        $this->result = sprintf( __( '%1$s removed from list "%2$s".', 'automatorwp-mailmint' ), $email, $name );
    }
...
new AutomatorWP_MailMint_Remove_From_List();
