(function( $ ) {
    ...
    var prefix = 'automatorwp-monday-';
    var _prefix = 'automatorwp_monday_';
    ...
    $('body').on('click', '.automatorwp_settings #' + _prefix + 'authorize', function(e) {
        ...
        if( consumer_key.length === 0 ) {
            ...
            response_wrap.html( 'All fields are required to connect with Monday' );
        ...
        $.post(
            ajaxurl,
            {
                action: 'automatorwp_monday_authorize',
                nonce: automatorwp_monday.nonce,
    ...
    // On change board
    // FIXME: Get the correct board id (really now is getting it empty)
    $('body').on('change', '.automatorwp-action-monday-change-item-name .cmb2-id-board select', function(e, first_change) {
