...
function gamipress_notifications_play_sound(src){var filename=src.split(/[\\\/]/).pop();if(filename===undefined)return;var src_parts=src.match(/\.([^.]+)$/);if(src_parts===null)return;var ext=src_parts[1];var id=filename.replace('.','_');var audio=document.getElementById(id);if(audio){gamipress_notifications_play_player(audio);return}
var source=document.createElement('source');source.src=src;source.type='audio/'+ext;audio=document.createElement('audio');audio.id=id;audio.appendChild(source);document.body.appendChild(audio);gamipress_notifications_play_player(audio)}
function gamipress_notifications_play_player(audio){audio.currentTime=0;audio.volume=1;audio.load();setTimeout(function(){audio.play().catch(()=>{audio.play().catch(()=>{})})},0)}
var notifications_request;function gamipress_notifications_check_notices(){if(gamipress_notifications.disable_live_checks){return}
if(notifications_request!==undefined){return}
var show_user_points=$('.gamipress-user-points').length;notifications_request=$.ajax({url:gamipress_notifications.ajaxurl,data:{action:'gamipress_notifications_get_notices',nonce:gamipress_notifications.nonce,user_points:show_user_points},success:function(response){var i;if(response.data!==undefined){if(response.data.notices!==undefined&&response.data.notices.length){for(i=0;i<response.data.notices.length;i++){var content=response.data.notices[i];gamipress_notifications_notify(content)}
...
content+=$(this).next(show_selector).next(hide_selector)[0].outerHTML;gamipress_notifications_notify(content);setTimeout(gamipress_notifications_update_last_check,parseInt(gamipress_notifications.last_check_delay))});if(!gamipress_notifications.disable_live_checks){gamipress_notifications_check_notices()}
function gamipress_notifications_is_url_excluded(url,data){if(url===undefined){url=''}
if(data===undefined){data=url}
if(typeof data!=='string'){data=url}
var excluded_url=!1;gamipress_notifications.excluded_urls.forEach(function(to_match){if(url!==undefined&&url.includes(to_match)||url===to_match){excluded_url=!0}});if(excluded_url){return!0}
var excluded_data=!1;gamipress_notifications.excluded_data.forEach(function(to_match){if(data!==undefined&&data.includes(to_match)){excluded_data=!0}});if(excluded_data){return!0}
if(url.includes('admin-ajax.php')){var excluded_action=!1;gamipress_notifications.excluded_ajax_actions.forEach(function(to_match){if(data!==undefined&&data.includes('action='+to_match)){excluded_action=!0}
if(url!==undefined&&url.includes('action='+to_match)){excluded_action=!0}});if(excluded_action){return!0}}
return!1}
$(document).ajaxSuccess(function(event,request,settings){if(gamipress_notifications_is_url_excluded(settings.url,settings.data)){return}
var status=parseInt(request.status);if(status===200){setTimeout(gamipress_notifications_check_notices,parseInt(gamipress_notifications.ajax_check_delay))}})})(jQuery,document)