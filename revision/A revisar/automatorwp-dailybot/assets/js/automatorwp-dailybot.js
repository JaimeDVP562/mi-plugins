(function( $ ) {
    ...
    var prefix  = 'automatorwp-dailybot-';
    var _prefix = 'automatorwp_dailybot_';
    ...
    $('body').on('click','#' + _prefix + 'authorize', function(e) {
        ...
        // Show error messagge if not correctly configured
        if( key.length === 0 ) {
            response_wrap.addClass( 'automatorwp-notice-error' );
            response_wrap.html( 'All fields are required to connect with Dailybot.' );
        ...
        $.post(
            ajaxurl,
            {
                action: 'automatorwp_dailybot_authorize',
                nonce: automatorwp_dailybot.nonce,
                key: key
            },
            function( response ) {
