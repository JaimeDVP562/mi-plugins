 ...
 *
 * @package     AutomatorWP\Integrations\Trello\Actions\Add-Label
...
class AutomatorWP_Trello_Add_Checklist_Item extends AutomatorWP_Integration_Action {

    public $integration = 'trello';
    public $action = 'trello_add_checklist_item';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add a checklist item in a card', 'automatorwp-trello'),
            'select_option'     => __( 'Add a <strong>checklist item</strong> in a card', 'automatorwp-trello'),
            /* translators: %1$s: Card. */
            'edit_label'        => sprintf( __( 'Add a new checklist item in %1$s', 'automatorwp-trello' ), '{card}' ),
            /* translators: %1$s: Card. */
            'log_label'         => sprintf( __( 'Add a new checklist item in %1$s', 'automatorwp-trello' ), '{card}' ),
            'options'           => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        ...
                        'checklist' => automatorwp_utilities_ajax_selector_field( array(
                            'name'              => __('Checklist: ', 'automatorwp-trello'),
                            'option_none'       => false,
                            'action_cb'         => 'automatorwp_trello_get_checklists',
                            'options_cb'        => 'automatorwp_trello_options_cb_checklist',
                            'placeholder'       => 'Select a Checklist',
                            'default'           => '',
                            'required'          => true
                        )),
                        'checklist_item' => array(
                                'name'          => __('New checklist item: ', 'automatorwp-trello'),
                                'type'          => 'text',
                                'default'       => '',
                                'required'      => true ,
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_trello_add_checklist_item( $action_options['checklist'], $action_options['checklist_item']  );
        
        if( $response === 200 ) {
            $this->result = __( 'Added Checklist item in card', 'autoamtorwp-trello' );
        }else {
            $this->result = __( 'The Checklist item could not be added', 'automatorwp-trello' );
        }
...
new AutomatorWP_Trello_Add_Checklist_Item();
