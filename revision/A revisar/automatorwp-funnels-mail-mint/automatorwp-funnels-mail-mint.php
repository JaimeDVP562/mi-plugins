 ...
 * Plugin URI:            https://automatorwp.com/add-ons/mailmint
...
final class AutomatorWP_Integration_Mailmint {
     ...
     * @access  public
     * @since   0.1.0
     * @return  AutomatorWP_Integration_Mailmint self::$instance The one true AutomatorWP_Integration_Mailmint
     */
    ...
    private function includes() {
        ...
        $force_load = (defined( 'AUTOMATORWP_MAILMINT_FORCE_LOAD_TRIGGERS' ) && AUTOMATORWP_MAILMINT_FORCE_LOAD_TRIGGERS===true)? true : false;
        if ( $this->meets_requirements() || $force_load ) { 
            $triggers_path = wp_normalize_path(AUTOMATORWP_MAILMINT_DIR . 'includes/triggers/*.php');
            $actions_path = wp_normalize_path(AUTOMATORWP_MAILMINT_DIR . 'includes/actions/*.php');
            
            foreach ( glob( $triggers_path ) as $file ) {
                require_once $file;
            }
            foreach ( glob( $actions_path ) as $file ) {
                require_once $file;
            }
...
/**
 * The main function responsible for returning the one true AutomatorWP_Integration_Mailmint
 * 
 * @since     0.1.0
 * @return    \AutomatorWP_Integration_Mailmint The one true AutomatorWP_Integration_Mailmint
 */
