 ...
 * Functions
 * 
 * @package AutomatorWP\Mailmint\Functions
 * @author  AutomatorWP
 * @since   0.1.0
 */
...
function automatorwp_mailmint_add_contact( $data ) {
    ...
    if ( function_exists( 'mailmint_create_single_contact' ) ) {
        ...
        if ( false !== strpos( $errstr, 'Undefined array key "status"' ) ) return true; // mark as handled to suppress the warning
        ...
        if ( is_array( $resp ) ) {
            ...
            if ( isset( $resp['status'] ) ) return in_array( strtolower( (string) $resp['status'] ), array( 'ok', 'success', 'created' ), true );
    ...
    if ( function_exists( 'mailmint_create_multiple_contacts' ) ) {
        ...
        $prev_handler = set_error_handler( function( $errno, $errstr ) {
            ...
            if ( false !== strpos( $errstr, 'Undefined array key "status"' ) ) return true;
        ...
        if ( is_array( $resp ) ) return ! empty( $resp );
