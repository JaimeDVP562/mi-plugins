 ...
 * List Contact
 *
 * @package     AutomatorWP\Integrations\Ontraport\Actions\List-Contact
...
class AutomatorWP_Ontraport_List_Contact extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'List a contact', 'automatorwp-ontraport'),
            'select_option'     => __( 'List a <strong>contact</strong>', 'automatorwp-ontraport'),
            /* translators: %1$s: Contact. */
            'edit_label'        => sprintf( __( 'List %1$s', 'automatorwp-ontraport' ), '{contact}' ),
            /* translators: %1$s: Contact. */
            'log_label'         => sprintf( __( 'List %1$s', 'automatorwp-ontraport' ) , '{contact}'),
            'options'           => array(
                'contact' => array(
                    'from' => 'contact',
                    ...
                    'fields' => array(
                        'id' => array(
                            'name'          => __('ID:', 'automatorwp-ontraport'),
                            'type'          => 'text',
                            'default'       => '',
                            'required'      => true
                        ),
    ...
    if ( $response === 200 ) {
        $this->result = __( 'Contact list successfully', 'automatorwp-ontraport' );
    } elseif ( $response === 404 ) {
        $this->result = __( 'Contact not found', 'automatorwp-ontraport' );
    } else {
        $this->result = __( 'Failed to list contact', 'automatorwp-ontraport' );
    }
...
new AutomatorWP_Ontraport_List_Contact();