...
class AutomatorWP_Keap_Create_Card extends AutomatorWP_Integration_Action {

    public $integration = 'keap';
    public $action = 'keap_create_card';
    ...
    public function register(){
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Create a new card', 'automatorwp-keap'),
            'select_option'     => __( 'Create a new <strong>card</strong>', 'automatorwp-keap'),
            /* translators: %1$s: Card. */
            'edit_label'        => sprintf( __( 'Create a new %1$s in %2$s', 'automatorwp-keap' ), '{card}', '{board}' ),
            /* translators: %1$s: Card. */
            'log_label'         => sprintf( __( 'Create a new %1$s in %2$s', 'automatorwp-keap' ) , '{card}', '{board}' ),
            'options'           => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        'card_title' => array(
                            'name'          => __('Card title:', 'automatorwp-keap'),
                            'type'          => 'text',
                            'default'       => '',
                            'required'      => true
                        ),
                        'card_desc' => array(
                            'name'          => __('Card description: ', 'automatorwp-keap'),
                            'type'          => 'textarea',
                            'required'      => false ,
                            'attributes'    => array(
                                'rows'      => '5'
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_keap_create_card( $card_name, $list_id, $card_desc );

        if( $response === 200 ) {
            $this->result = __( 'Created card in list', 'autoamtorwp-keap' );
        }else {
            $this->result = __( 'The card could not be created', 'automatorwp-keap' );
        }
...
new AutomatorWP_Keap_Create_Card();