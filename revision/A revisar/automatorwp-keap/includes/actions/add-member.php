...
class AutomatorWP_Keap_Add_Member extends AutomatorWP_Integration_Action {

    public $integration = 'keap';
    public $action = 'keap_add_member';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add a member in a card', 'automatorwp-keap'),
            'select_option'     => __( 'Add a <strong>member</strong> in a card', 'automatorwp-keap'),
             /* translators: %1$s: Card. */
             'edit_label'        => sprintf( __( 'Add a new member in %1$s', 'automatorwp-keap' ), '{card}' ),
             /* translators: %1$s: Card. */
             'log_label'         => sprintf( __( 'Add a new member in %1$s', 'automatorwp-keap' ), '{card}' ),
             'options'           => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        ...
                        'member' => automatorwp_utilities_ajax_selector_field( array(
                            'name'              => __('New member: ', 'automatorwp-keap'),
                            'option_none'       => false,
                            'action_cb'         => 'automatorwp_keap_get_members',
                            'options_cb'        => 'automatorwp_keap_options_cb_member',
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_keap_add_member( $action_options['card'], $action_options['member'] );

        if( $response === 200 ) {
            $this->result = __( 'Added member in card', 'autoamtorwp-keap' );
        }else {
            $this->result = __( 'The member could not be added', 'automatorwp-keap' );
        }
...
new AutomatorWP_Keap_Add_Member();