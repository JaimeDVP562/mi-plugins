 ...
 * Delete Card
 *
 * @package     AutomatorWP\Integrations\Keap\Actions\Delete-Card
...
class AutomatorWP_Keap_Delete_Card extends AutomatorWP_Integration_Action {

    public $integration = 'keap';
    public $action = 'keap_delete_card';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Delete a card', 'automatorwp-keap'),
            'select_option'     => __( 'Delete a <strong>card</strong>', 'automatorwp-keap'),
            /* translators: %1$s: Card. */
            'edit_label'        => sprintf( __( 'Delete %1$s', 'automatorwp-keap' ), '{card}' ),
            /* translators: %1$s: Card. */
            'log_label'         => sprintf( __( 'Delete %1$s', 'automatorwp-keap' ) , '{card}'),
            'options'           => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        ...
                        'permanently_delete' => array(
                            'name'              => __('Delete the card permanently?', 'automatorwp-keap'),
                            'desc'              => __('Check this option to delete permanently the card. Otherwise it will be archived.', 'automatorwp-keap'),
                            'type'              => 'checkbox',
                            'classes'           => 'cmb2-switch'
                        )
                    )
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        if( empty( $permanently_delete ) ) {
            $response = automatorwp_keap_archive_card( $card_id );
        }else{
            $response = automatorwp_keap_delete_card( $card_id );
        }

        if( $response === 200 ) {
            if( empty( $permanently_delete ) ) {
                $this->result = __( 'Card archived.', 'autoamtorwp-keap' );
            }else{
                $this->result = __( 'Card deleted.', 'autoamtorwp-keap' );
            }
        }else {
            $this->result = __( 'The card could not be deleted', 'automatorwp-keap' );
        }
    }
...
new AutomatorWP_Keap_Delete_Card();