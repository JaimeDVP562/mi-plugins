 ...
 * Comment Card
 *
 * @package     AutomatorWP\Integrations\Keap\Actions\Comment-Card
...
class AutomatorWP_Keap_Comment_Card extends AutomatorWP_Integration_Action {

    public $integration = 'keap';
    public $action = 'keap_comment_card';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add a comment in a card', 'automatorwp-keap'),
            'select_option'     => __( 'Add a <strong>comment</strong> in a card', 'automatorwp-keap'),
            /* translators: %1$s: Comment. */
            'edit_label'        => sprintf( __( 'Add a new %1$s in %2$s', 'automatorwp-keap' ), '{comment}', '{card}' ),
            /* translators: %1$s: Comment. */
            'log_label'         => sprintf( __( 'Add a new %1$s in %2$s', 'automatorwp-keap' ) , '{comment}', '{card}' ),
            'options'           => array(
                'comment' => array(
                    'from' => 'comment',
                    'default' => __('comment', 'automatorwp-keap'),
                    'fields' => array(
                        'new_comment' => array(
                            'name'          => __('New comment: ', 'automatorwp-keap'),
                            'type'          => 'textarea',
                            'required'      => true ,
                            'attributes'    => array(
                                'rows'      => '5'
                ...
                'card' => array(
                    'from' => 'card',
                    'default' => __('card from list', 'automatorwp-keap'),
                    'fields' => array(
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_keap_comment_card( $action_options['card'], $action_options['new_comment']  );
        
        if( $response === 200 ) {
            $this->result = __( 'Added comment in card', 'autoamtorwp-keap' );
        }else {
            $this->result = __( 'The comment could not be added', 'automatorwp-keap' );
        }
...
new AutomatorWP_Keap_Comment_Card();