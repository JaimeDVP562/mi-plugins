...
class AutomatorWP_Keap_Change_Desc extends AutomatorWP_Integration_Action {

    public $integration = 'keap';
    public $action = 'keap_change_desc';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Change a card description', 'automatorwp-keap'),
            'select_option'     => __( 'Change a card <strong>description</strong>', 'automatorwp-keap'),
            /* translators: %1$s: Card. */
            'edit_label'        => sprintf( __( 'Change %1$s description', 'automatorwp-keap' ), '{card}' ),
            /* translators: %1$s: Card. */
            'log_label'         => sprintf( __( 'Change %1$s description', 'automatorwp-keap' ) , '{card}'),
            'options'           => array(
                'card' => array(
                    'from' => 'card',
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_keap_change_card_desc( $card_id, $new_desc );

        if( $response === 200 ) {
            $this->result = __( 'Changed card description', 'autoamtorwp-keap' );
        }else {
            $this->result = __( 'The card description could not be changed', 'automatorwp-keap' );
        }
    }
...
new AutomatorWP_Keap_Change_Desc();