/**
 * Grok Integration Script
 */
(function ($) {
    'use strict';

    // Prefix used for Grok integration
    var _prefix = 'automatorwp_grok_';
    ...
    $('body').on('click', '#' + _prefix + 'authorize_btn', function (e) {
        ...
        if (!token || token.trim().length === 0) {
            response_wrap.attr('class', 'automatorwp-notice-error')
                .html('API token is required to connect with Grok')
                .slideDown('fast');
            return;
        }
        ...
        $.post(
            automatorwp_grok.ajax_url,
            {
                action: 'automatorwp_grok_authorize',
                nonce: automatorwp_grok.nonce,
                token: token
            },
            function (response) {
