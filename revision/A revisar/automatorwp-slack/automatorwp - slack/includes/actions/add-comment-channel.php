 ...
 * Add Comment Task
 *
 * @package     AutomatorWP\Integrations\Slack\Actions\Add_Comment_Channel
...
class AutomatorWP_Slack_Add_Comment_Channel extends AutomatorWP_Integration_Action {

    public $integration = 'slack';
    public $action = 'slack_add_comment_channel';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add comment to channel', 'automatorwp-slack' ),
            'select_option'     => __( 'Add comment to <strong>channel</strong>', 'automatorwp-slack' ),
            /* translators: %1$s: Channel. */
            'edit_label'        => sprintf( __( 'Add comment to channel %1$s', 'automatorwp-slack' ), '{channel}' ),
            /* translators: %1$s: Channel. */
            'log_label'         => sprintf( __( 'Add comment to channel  %1$s', 'automatorwp-slack' ), '{channel}' ),
            'options'           => array(
                'channel' => array(
                    'from' => 'channel',
                    ...
                    'fields' => array(
                        ...
                        'channel_custom' => automatorwp_utilities_custom_field( array(
                            'option_custom_desc'    => __( 'Id or name of the channel', 'automatorwp-slack' ),
                        ) ),
                        'comment' => array(
                            'name' => __( 'Comment:', 'automatorwp-slack' ),
                            'desc' => __( 'The comment to add to the channel', 'automatorwp-slack' ),
                            'type' => 'wysiwyg',
                            'required'  => true,
                            'default' => ''
                        ),
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_slack_add_comment( $channel_id, $comment_text );
        
        // Create comment
        if ( $response['ok'] === 200 ) {
            $this->result = __( 'Created comment in channel ', 'automatorwp-slack' );
        } else {
...
new AutomatorWP_Slack_Add_Comment_Channel();