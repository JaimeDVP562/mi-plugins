...
class AutomatorWP_Slack_Add_Comment_Thread extends AutomatorWP_Integration_Action {

    public $integration = 'slack';
    public $action = 'slack_add_comment_thread';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add comment to a thread', 'automatorwp-slack' ),
            'select_option'     => __( 'Add comment to a <strong>thread</strong>', 'automatorwp-slack' ),
            /* translators: %1$s: Thread. */
            'edit_label'        => sprintf( __( 'Add text  %1$s to thread', 'automatorwp-slack' ), '{comment}' ),
            /* translators: %1$s: Thread. */
            'log_label'         => sprintf( __( 'Add text  %1$s to thread', 'automatorwp-slack' ), '{comment}' ),
            'options'           => array(
                'comment' => array(
                    'from' => 'comment',
                    'default' => __( 'comment', 'automatorwp-slack' ),
                    'fields' => array(
                        'channel_text' => array(
                            'name' => __( 'Channel:', 'automatorwp-slack' ),
                            'desc' => __( 'The channel name or channel id', 'automatorwp-slack' ),
                            'type' => 'text',
                            'required'  => true,
                            'default' => '',
                        ...
                        'thread' => array(
                            'name' => __( 'Thread id:', 'automatorwp-slack' ),
                            'desc' => __( 'The thread id', 'automatorwp-slack' ),
                            'type' => 'text',
                            'required'  => true,
                            'default' => '',
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        // Shorthand
        $channel_id = $action_options['channel_text'];
        ...
        $channel_id = automatorwp_slack_get_channel_id( $action_options['channel_text'] );
...
new AutomatorWP_Slack_Add_Comment_Thread();