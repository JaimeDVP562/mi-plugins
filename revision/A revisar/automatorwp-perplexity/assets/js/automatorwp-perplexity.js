(function ($) {
  var _prefix = "automatorwp_perplexity_";
  ...
  $("body").on(
    ...
    function (e) {
      ...
      // Show error if api_key is empty
      if (api_key.length === 0) {
        response_wrap.addClass("automatorwp-notice-error");
        response_wrap.html("API Key is required to connect with Perplexity.");
      ...
      $.post(
        ajaxurl,
        {
          action: "automatorwp_perplexity_authorize",
          nonce: automatorwp_perplexity.nonce,
          api_key: api_key,
        },
        function (response) {
