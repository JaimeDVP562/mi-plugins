 ...
 * Deep Research
 *
 * @package     AutomatorWP\Integrations\Perplexity\Actions\Deep_Research
 * @since       1.0.0
 */
// Exit if accessed directly
if ( ! defined( 'ABSPATH' ) ) exit;

class AutomatorWP_Perplexity_Deep_Research extends AutomatorWP_Integration_Action
{

    public $integration = 'perplexity';
    public $action      = 'perplexity_deep_research';
    ...
    public function register()
    {
        automatorwp_register_action(
            ...
            array(
                ...
                'label'         => __( 'Run a deep research on Perplexity', 'automatorwp-perplexity' ),
                'select_option' => __( 'Run a <strong>deep research</strong> on Perplexity', 'automatorwp-perplexity' ),
                'edit_label'    => __( 'Run deep research on {research_query} with Perplexity', 'automatorwp-perplexity' ),
                'log_label'     => __( 'Run a deep research on Perplexity', 'automatorwp-perplexity' ),
                'options'       => array(
                    'research_query' => array(
                        'from'    => 'research_query',
                        'default' => __( 'topic', 'automatorwp-perplexity' ),
                        'fields'  => array(
                            'research_query'        => array(
                                'name'     => __( 'Research Query', 'automatorwp-perplexity' ),
                                'desc'     => __( 'Topic or question to research in depth. Supports tags.', 'automatorwp-perplexity' ),
                                'type'     => 'textarea',
                                'default'  => '',
                                'required' => true,
                            ),
                            'system_message'        => array(
                                ...
                                'desc'    => __( '(Optional) Researcher role (e.g. "You are a financial analyst"). Supports tags.', 'automatorwp-perplexity' ),
                                'type'    => 'textarea',
                                'default' => '',
                            ),
                ...
                'tags'          => array(
                    'perplexity_research_report'          => array(
                        'label'   => __( 'Perplexity Research Report', 'automatorwp-perplexity' ),
                        'type'    => 'text',
                        'preview' => __( 'Full research report', 'automatorwp-perplexity' ),
                    ),
                    'perplexity_research_search_results'  => array(
                        'label'   => __( 'Perplexity Research Search Results', 'automatorwp-perplexity' ),
                        'type'    => 'text',
    ...
    {
        $research_query        = isset( $action_options['research_query'] )        ? $action_options['research_query']                          : '';
        ...
        $max_tokens            = isset( $action_options['max_tokens'] )            ? (int) $action_options['max_tokens']                        : 2048;
        ...
        if ( empty( $research_query ) ) {
            $this->result = __( 'Research query field is empty.', 'automatorwp-perplexity' );
            return;
        }
        ...
        $messages[] = array( 'role' => 'user', 'content' => $research_query );
        ...
        $response = automatorwp_perplexity_api_request( 'sonar-deep-research', $messages, $api_args );
        ...
        automatorwp_update_action_tag( $action->ID, 'perplexity_research_report',         $e['text'] );
        automatorwp_update_action_tag( $action->ID, 'perplexity_research_search_results',  $e['search_results'] );

        if ( ! empty( $response_tag ) ) {
            automatorwp_update_action_tag( $action->ID, $response_tag, $e['text'] );
        }

        $this->result = sprintf( __( 'Deep research completed. Report length: %d characters.', 'automatorwp-perplexity' ), strlen( $e['text'] ) );
    }
...
new AutomatorWP_Perplexity_Deep_Research();
