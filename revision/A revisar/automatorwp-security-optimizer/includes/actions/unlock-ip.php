...
/**
 * AutomatorWP action class for unblocking a user IP in SG Security.
 */
class AutomatorWP_SG_Security_Unblock_IP extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        automatorwp_register_action( $this->action, array(
            ...
            'label'         => __( 'Unblock a user by IP', 'automatorwp' ),
            'select_option' => __( 'Unblock <strong>user</strong> by IP', 'automatorwp' ),
            'edit_label'    => sprintf( __( 'Unblock user %1$s by IP', 'automatorwp' ), '{user_id}' ),
            'log_label'     => sprintf( __( 'User %1$s IP unblocked', 'automatorwp' ), '{user_id}' ),
            'options'       => array(
                'user_id' => array(
                    'from' => 'user_id',
                    'fields' => array(
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        if ( class_exists( '\SG_Security\Block_Service\Block_Service' ) ) {
            ...
            if ( ! empty( $response['result'] ) && 1 === intval( $response['result'] ) ) {
                ...
                if ( false === $updated ) {
                    return new WP_Error( 'unblock_failed', __( 'Could not unblock the IP.', 'automatorwp' ) );
                }
