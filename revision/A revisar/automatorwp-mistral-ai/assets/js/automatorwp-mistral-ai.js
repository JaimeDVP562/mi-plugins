(function( $ ) {
    ...
    var _prefix = 'automatorwp_mistral_ai_';
    ...
    $('body').on('click', '.automatorwp_settings #' + _prefix + 'authorize', function(e) {
        ...
        if( ! token || token.length === 0 ) {
            ...
            response_wrap.html('API token is required to connect with Mistral AI');
        ...
        $.post(
            ajaxurl,
            {
                action: 'automatorwp_mistral_authorize',
                nonce: automatorwp_mistral_ai.nonce,
                token: token
            },
            function( response ) {

                response_wrap.addClass( 'automatorwp-notice-' + ( response.success === true ? 'success' : 'error' ) );
                response_wrap.html( ( response.data && response.data.message !== undefined ? response.data.message : response.data ) );
                response_wrap.slideDown('fast');

                wrapper.find('.spinner').remove();

                if( response.success === true && response.data && response.data.redirect_url !== undefined ) {
