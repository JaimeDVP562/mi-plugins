...
class AutomatorWP_Smoove_Add_Label extends AutomatorWP_Integration_Action {

    public $integration = 'smoove';
    public $action = 'smoove_add_label';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add a label in a card', 'automatorwp-smoove'),
            'select_option'     => __( 'Add a <strong>label</strong> in a card', 'automatorwp-smoove'),
            /* translators: %1$s: Card. */
            'edit_label'        => sprintf( __( 'Add a new label in %1$s', 'automatorwp-smoove' ), '{card}' ),
            /* translators: %1$s: Card. */
            'log_label'         => sprintf( __( 'Add a new label in %1$s', 'automatorwp-smoove' ), '{card}' ),
            'options'           => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        ...
                        'label' => automatorwp_utilities_ajax_selector_field( array(
                            'name'              => __('New label: ', 'automatorwp-trello'),
                            'option_none'       => false,
                            'action_cb'         => 'automatorwp_trello_get_labels',
                            'options_cb'        => 'automatorwp_trello_options_cb_label',
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_trello_add_label( $action_options['card'], $action_options['label'] );

        if( $response === 200 ) {
            $this->result = __( 'Added label in card', 'autoamtorwp-trello' );
        }else {
            $this->result = __( 'The label could not be added', 'automatorwp-trello' );
        }
...
new AutomatorWP_Trello_Add_Label();