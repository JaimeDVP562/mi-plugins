...
class AutomatorWP_Smoove_Create_Contacts extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Create a new contact', 'automatorwp-smoove'),
            'select_option'     => __( 'Create a new <strong>contact</strong>', 'automatorwp-smoove'),
            'edit_label'        => sprintf( __( 'Create a new %1$s', 'automatorwp-smoove' ), '{contact}' ),
            'log_label'         => sprintf( __( 'Created a new %1$s', 'automatorwp-smoove' ) , '{contact}' ),
            'options'           => array(
                'contact' => array(
                    'from' => 'contact',
                    ...
                    'fields' => array(
                        'contact_request' => array(
                            'name'          => __('Contact data:', 'automatorwp-smoove'),
                            'type'          => 'textarea',
                            'default'       => '',
                            'required'      => true
                        ),
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        // Shorthand
        $contacts_data = $action_options['contact_request'];
        ...
        // Call API to create contacts in Smoove
        $response = automatorwp_smoove_create_contact( json_decode($contacts_data, true) );

        // Check API response
        if( $response === 200 ) {
            $this->result = __( 'Contact created successfully in Smoove', 'automatorwp-smoove' );
        } else {
            $this->result = __( 'Failed to create contact in Smoove', 'automatorwp-smoove' );
        }
    }
