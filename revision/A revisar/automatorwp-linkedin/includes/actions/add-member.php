 ...
 *
 * @package     AutomatorWP\Integrations\Trello\Actions\Add-Member
...
class AutomatorWP_Trello_Add_Member extends AutomatorWP_Integration_Action
{

    public $integration = 'trello';
    public $action = 'trello_add_member';
    ...
    public function register()
    {
        ...
        automatorwp_register_action($this->action, array(
            ...
            'label' => __('Add a member in a card', 'automatorwp-trello'),
            'select_option' => __('Add a <strong>member</strong> in a card', 'automatorwp-trello'),
            /* translators: %1$s: Card. */
            'edit_label' => sprintf(__('Add a new member in %1$s', 'automatorwp-trello'), '{card}'),
            /* translators: %1$s: Card. */
            'log_label' => sprintf(__('Add a new member in %1$s', 'automatorwp-trello'), '{card}'),
            'options' => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        ...
                        'member' => automatorwp_utilities_ajax_selector_field(array(
                            'name' => __('New member: ', 'automatorwp-trello'),
                            'option_none' => false,
                            'action_cb' => 'automatorwp_trello_get_members',
                            'options_cb' => 'automatorwp_trello_options_cb_member',
    ...
    public function execute($action, $user_id, $action_options, $automation)
    {
        ...
        $response = automatorwp_trello_add_member($action_options['card'], $action_options['member']);

        if ($response === 200) {
            $this->result = __('Added member in card', 'automatorwp-trello');
        } else {
            $this->result = __('The member could not be added', 'automatorwp-trello');
        }
...
new AutomatorWP_Trello_Add_Member();