 ...
 * Create Card
 *
 * @package     AutomatorWP\Integrations\Trello\Actions\Change-Desc
...
class AutomatorWP_Trello_Change_Desc extends AutomatorWP_Integration_Action
{

    public $integration = 'trello';
    public $action = 'trello_change_desc';
    ...
    public function register()
    {
        ...
        automatorwp_register_action($this->action, array(
            ...
            'label' => __('Change a card description', 'automatorwp-trello'),
            'select_option' => __('Change a card <strong>description</strong>', 'automatorwp-trello'),
            /* translators: %1$s: Card. */
            'edit_label' => sprintf(__('Change %1$s description', 'automatorwp-trello'), '{card}'),
            /* translators: %1$s: Card. */
            'log_label' => sprintf(__('Change %1$s description', 'automatorwp-trello'), '{card}'),
            'options' => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        ...
                        'desc' => array(
                            ...
                            'desc' => 'Write the new description here.',
                            'type' => 'textarea',
                            'required' => false,
                            'default' => '',
                            'attributes' => array(
    ...
    public function execute($action, $user_id, $action_options, $automation)
    {
        ...
        $response = automatorwp_trello_change_card_desc($card_id, $new_desc);

        if ($response === 200) {
            $this->result = __('Changed card description', 'automatorwp-trello');
        } else {
            $this->result = __('The card description could not be changed', 'automatorwp-trello');
        }
    }
...
new AutomatorWP_Trello_Change_Desc();