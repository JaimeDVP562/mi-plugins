 ...
 * Unlock IP
 *
 * @package     AutomatorWP\Security_Optimizer\Actions\Unlock_IP
 * @since       1.0.0
 */
// Exit if accessed directly
if( !defined( 'ABSPATH' ) ) exit;

class AutomatorWP_SG_Security_Unblock_IP extends AutomatorWP_Integration_Action {

    public $integration = 'security_optimizer';
    public $action = 'sg_security_unblock_ip';

    /**
     * Registers the IP unblock action with AutomatorWP.
     *
     * @since 1.0.0
     */
    public function register() {
        automatorwp_register_action( $this->action, array(
            ...
            'label'         => __( 'Unblock a user by IP', 'automatorwp-security-optimizer' ),
            'select_option' => __( 'Unblock <strong>user</strong> by IP', 'automatorwp-security-optimizer' ),
            'edit_label'    => sprintf( __( 'Unblock user %1$s by IP', 'automatorwp-security-optimizer' ), '{user_id}' ),
            'log_label'     => sprintf( __( 'User %1$s IP unblocked', 'automatorwp-security-optimizer' ), '{user_id}' ),
            'options'       => array(
                'user_id' => array(
                    'from'   => 'user_id',
                    'fields' => array(
    ...
    /**
     * Executes the IP unblock action via SG Security.
     *
     * @since 1.0.0
     *
     * @param object $action
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        if ( class_exists( '\SG_Security\Block_Service\Block_Service' ) ) {
            ...
            $response = $block_service->block_ip( $visitor_id, 0 );
            ...
            if ( ! empty( $response['result'] ) && 1 === intval( $response['result'] ) ) {
                ...
                $this->result = __( 'IP unblocked successfully.', 'automatorwp-security-optimizer' );
                return;
            }
        }
        ...
        if ( false === $updated ) {
            $this->result = __( 'Could not unblock the IP.', 'automatorwp-security-optimizer' );
            return;
        }
...
new AutomatorWP_SG_Security_Unblock_IP();