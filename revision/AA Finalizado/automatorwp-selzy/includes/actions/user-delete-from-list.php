 ...
 *
 * @package     AutomatorWP\Integrations\Selzy\Actions\User_Delete_From_List
...
class AutomatorWP_Selzy_Del_User extends AutomatorWP_Integration_Action {

    public $integration = 'selzy';
    public $action = 'selzy_del_user';
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Delete user from list', 'automatorwp-selzy' ),
            'select_option'     => __( 'Delete <strong>user</strong> from <strong>list</strong>', 'automatorwp-selzy' ),
            /* translators: %1$s: List. */
            'edit_label'        => sprintf( __( 'Delete user from %1$s', 'automatorwp-selzy' ), '{list}' ),
            /* translators: %1$s: List. */
            'log_label'         => sprintf( __( 'Delete user from %1$s', 'automatorwp-selzy' ), '{list}' ),
            'options'           => array(
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        // Bail if user is out of list
        if(!automatorwp_selzy_check_contact($user_email, $list_id )) {
            $this->result = __( 'Contact isnt in list already.', 'automatorwp-selzy' );
            return;
        }

        $response = automatorwp_selzy_del_contact( $user_email, $list_id );

        if ( $response === 200 ) {
            $this->result = __( 'Deleted user from list', 'automatorwp-selzy' );
        } else {
            $this->result = __( 'Couldnt delete user from list', 'automatorwp-selzy' );
        }
...
new AutomatorWP_Selzy_Del_User();