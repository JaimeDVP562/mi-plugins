 ...
 * Create Contact
 *
 * @package     AutomatorWP\Integrations\Mailjet\Actions\Create-Contact
...
class AutomatorWP_Mailjet_Create_Contact extends AutomatorWP_Integration_Action
{

    public $integration = 'mailjet';
    public $action = 'mailjet_create_contact';
    ...
    public function register()
    {
        ...
        automatorwp_register_action(
            ...
            array(
                ...
                'label'             => __('Create a new contact', 'automatorwp-mailjet'),
                'select_option'     => __('Create a new <strong>contact</strong>', 'automatorwp-mailjet'),
                /* translators: %1$s: Contact. */
                'edit_label'        => sprintf(__('Create a new %1$s', 'automatorwp-mailjet'), '{contact}'),
                /* translators: %1$s: Contact. */
                'log_label'         => sprintf(__('Create a new %1$s', 'automatorwp-mailjet'), '{contact}'),
                'options'           => array(
                    'contact' => array(
                        'from' => 'contact',
    ...
    public function execute($action, $user_id, $action_options, $automation)
    {
        ...
        // Bail if first name or email is empty
        ...
        $response = automatorwp_mailjet_create_contact($first_name, $email);

        if ($response === 200) {
            $this->result = __('Created contact', 'automatorwp-mailjet');
        } else {
            $this->result = __('The contact could not be created', 'automatorwp-mailjet');
        }
    }
...
new AutomatorWP_Mailjet_Create_Contact();
