 ...
 * Create Card
 *
 * @package     AutomatorWP\Integrations\Trello\Actions\Create-Card
...
class AutomatorWP_Trello_Create_Card extends AutomatorWP_Integration_Action {

    public $integration = 'trello';
    public $action = 'trello_create_card';
    ...
    public function register(){
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Create a new card', 'automatorwp-trello'),
            'select_option'     => __( 'Create a new <strong>card</strong>', 'automatorwp-trello'),
            /* translators: %1$s: Card. */
            'edit_label'        => sprintf( __( 'Create a new %1$s in %2$s', 'automatorwp-trello' ), '{card}', '{board}' ),
            /* translators: %1$s: Card. */
            'log_label'         => sprintf( __( 'Create a new %1$s in %2$s', 'automatorwp-trello' ) , '{card}', '{board}' ),
            'options'           => array(
                'card' => array(
                    'from' => 'card',
                    ...
                    'fields' => array(
                        'card_title' => array(
                            'name'          => __('Card title:', 'automatorwp-trello'),
                            'type'          => 'text',
                            'default'       => '',
                            'required'      => true
                        ),
                        'card_desc' => array(
                            'name'          => __('Card description: ', 'automatorwp-trello'),
                            'type'          => 'textarea',
                            'required'      => false ,
                            'attributes'    => array(
                                'rows'      => '5'
                ...
                'board' => array(
                    'from' => 'board',
                    'default' => __('list from board', 'automatorwp-trello'),
                    'fields' => array(
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        $response = automatorwp_trello_create_card( $card_name, $list_id, $card_desc );

        if( $response === 200 ) {
            $this->result = __( 'Created card in list', 'autoamtorwp-trello' );
        }else {
            $this->result = __( 'The card could not be created', 'automatorwp-trello' );
        }
...
new AutomatorWP_Trello_Create_Card();