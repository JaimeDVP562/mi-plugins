...
function automatorwp_keap_settings_sections( $automatorwp_settings_sections ) {
	...
	$automatorwp_settings_sections['keap'] = array(
		...
		'icon'  => 'dashicons-email-alt', // Changed icon to email-alt as dashicons-keap doesn't exist
	);
...
function automatorwp_keap_settings_meta_boxes( $meta_boxes ) {
	...
	$meta_boxes['automatorwp-keap-settings'] = array(
		'title'  => __( 'Keap Settings', 'automatorwp-keap' ),
		'fields' => apply_filters( 'automatorwp_keap_settings_fields', array(
			$prefix . 'access_token' => array(
				'name' => __( 'Access Token / API Key:', 'automatorwp-keap' ),
				'desc' => __( 'Enter your Keap Personal Access Token or API Key.', 'automatorwp-keap' ),
				'type' => 'text',
			),
...
function automatorwp_keap_authorize_display_cb( $field_args, $field ) {
    ...
    <div class="cmb-row cmb-type-custom cmb2-id-automatorwp-keap-authorize table-layout" data-fieldtype="custom">
        ...
        <div class="cmb-td">
            <input type="hidden" name="awp-keap-outh-ajax-nonce" id="awp-keap-outh-ajax-nonce"
                   value="<?php echo wp_create_nonce( 'automatorwp_keap_authorize_nonce' ); ?>"/>
            <input type="button" name="automatorwp_save_keap_oauth" id="<?php echo $field_id; ?>"
                   value="<?php _e( 'Save Credentials', 'automatorwp-keap' ); ?>" class="button button-primary"/>
			<?php if ( $access_valid ) { ?>
                <input type="button" name="automatorwp_remove_keap_oauth" id="automatorwp_remove_keap_oauth"
                       value="<?php _e( 'Disconnect', 'automatorwp-keap' ); ?>" class="button button-danger"/><br>
			<?php } ?>
