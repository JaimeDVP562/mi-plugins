 ...
 * User Is Not Active
 *
 * @package     AutomatorWP\Integrations\MemberPress\Filters\User_Is_Not_Active
...
class AutomatorWP_MemberPress_User_Is_Not_Active_Filter extends AutomatorWP_Integration_Filter {

    public $integration = 'memberpress';
    public $filter = 'memberpress_user_is_not_active';
    ...
    public function register() {
        ...
        automatorwp_register_filter( $this->filter, array(
            ...
            'label'             => __( 'User is not active in membership', 'automatorwp-memberpress' ),
            'select_option'     => __( 'User <strong>is not active</strong> in membership', 'automatorwp-memberpress' ),
    ...
    public function user_deserves_filter( $deserves_filter, $filter, $user_id, $event, $filter_options, $automation ) {
        ...
        if ( $membership === 'any' ) {
            // Don't deserve if user is in any membership
            if( !empty( $active_subscriptions ) ) {
                $this->result = sprintf( __( 'Filter not passed. User is active in membership "%1$s".', 'automatorwp-memberpress' ),
                    $membership_name
                );
                return false;
            }
        } else{
            // Don't deserve if user is in this membership
        ...
        $this->result = sprintf( __( 'Filter passed. User is not active in membership "%1$s".', 'automatorwp-memberpress' ),
            $membership_name
        );
...
new AutomatorWP_MemberPress_User_Is_Not_Active_Filter();