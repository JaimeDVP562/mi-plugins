<?php
/**
 * Trigger: SendPulse - Received Email
 *
...
class AutomatorWP_SendPulse_Received_Email_Trigger extends AutomatorWP_Integration_Trigger {
    public $integration = 'sendpulse';
    public $trigger     = 'sendpulse_received_email';

    public function register() {
        automatorwp_register_trigger( $this->trigger, array(
            'integration'   => $this->integration,
            'label'         => __( 'A SendPulse subscriber receives an email', 'automatorwp-sendpulse' ),
            'select_option' => __( 'A SendPulse subscriber <strong>receives an email</strong>', 'automatorwp-sendpulse' ),
            'edit_label'    => sprintf( __( 'A SendPulse subscriber receives an email %1$s time(s)', 'automatorwp-sendpulse' ), '{times}' ),
            'log_label'     => __( 'A SendPulse subscriber receives an email', 'automatorwp-sendpulse' ),
            'action'        => 'automatorwp_sendpulse_received', // EVENTO: received
            ...
            'tags'          => array_merge( array(
                ...
                'sendpulse_subject' => array( 'label' => __( 'Email Subject', 'automatorwp-sendpulse' ), 'type' => 'text' ),
    ...
    public function listener( $data ) {
        ...
        $subject = isset( $data[0]['subject'] ) ? sanitize_text_field( $data[0]['subject'] ) : '';
...
new AutomatorWP_SendPulse_Received_Email_Trigger();