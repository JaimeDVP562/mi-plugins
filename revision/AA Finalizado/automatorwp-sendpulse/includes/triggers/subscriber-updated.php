<?php
/**
 * Trigger: SendPulse - Subscriber Updated
 *
...
class AutomatorWP_SendPulse_Subscriber_Updated_Trigger extends AutomatorWP_Integration_Trigger {
    public $integration = 'sendpulse';
    public $trigger     = 'sendpulse_subscriber_updated';

    public function register() {
        automatorwp_register_trigger( $this->trigger, array(
            'integration'   => $this->integration,
            'label'         => __( 'A SendPulse subscriber is updated', 'automatorwp-sendpulse' ),
            'select_option' => __( 'A SendPulse subscriber <strong>is updated</strong>', 'automatorwp-sendpulse' ),
            'edit_label'    => sprintf( __( 'A SendPulse subscriber is updated %1$s time(s)', 'automatorwp-sendpulse' ), '{times}' ),
            'log_label'     => __( 'A SendPulse subscriber is updated', 'automatorwp-sendpulse' ),
            'action'        => 'automatorwp_sendpulse_update', // EVENTO: update
            ...
            'tags'          => array_merge( array(
                ...
                'sendpulse_name'  => array( 'label' => __( 'Subscriber Name', 'automatorwp-sendpulse' ), 'type' => 'text' ),
    ...
    public function listener( $data ) {
        ...
        $name  = isset( $data[0]['name'] ) ? sanitize_text_field( $data[0]['name'] ) : '';
...
new AutomatorWP_SendPulse_Subscriber_Updated_Trigger();