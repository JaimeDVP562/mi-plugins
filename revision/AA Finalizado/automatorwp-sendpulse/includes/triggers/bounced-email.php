<?php
/**
 * Trigger: SendPulse - Bounced Email
 */
if ( ! defined( 'ABSPATH' ) ) exit;

if ( ! class_exists( 'AutomatorWP_Integration_Trigger' ) ) return;

class AutomatorWP_SendPulse_Bounced_Email_Trigger extends AutomatorWP_Integration_Trigger {
    public $integration = 'sendpulse';
    public $trigger     = 'sendpulse_bounced_email';

    public function register() {
        automatorwp_register_trigger( $this->trigger, array(
            'integration'   => $this->integration,
            'label'         => __( 'A SendPulse subscriber email bounces', 'automatorwp-sendpulse' ),
            'select_option' => __( 'A SendPulse subscriber email <strong>bounces</strong>', 'automatorwp-sendpulse' ),
            'edit_label'    => sprintf( __( 'A SendPulse subscriber email bounces %1$s time(s)', 'automatorwp-sendpulse' ), '{times}' ),
            'log_label'     => __( 'A SendPulse subscriber email bounces', 'automatorwp-sendpulse' ),
            'action'        => 'automatorwp_sendpulse_bounce', // EVENTO: bounce
...
new AutomatorWP_SendPulse_Bounced_Email_Trigger();