...
function automatorwp_woocommerce_get_tags( $tags ) {
    ...
    $tags['woocommerce_billing'] = array(
        'label' => __( 'Customer Billing Address', 'automatorwp-woocommerce' ),
        'tags'  => array(),
    ...
    $tags['woocommerce_shipping'] = array(
        'label' => __( 'Customer Shipping Address', 'automatorwp-woocommerce' ),
        'tags'  => array(),
        'icon'  => AUTOMATORWP_WOOCOMMERCE_URL . 'assets/woocommerce.svg',
    );
...
function automatorwp_woocommerce_order_tags() {
    ...
    $order_tags = array(
        'woocommerce_order_id' => array(
            'label'     => __( 'Order ID', 'automatorwp-woocommerce' ),
            'type'      => 'integer',
            'preview'   => '123',
        ),
        'woocommerce_order_number' => array(
            'label'     => __( 'Order Number', 'automatorwp-woocommerce' ),
            'type'      => 'integer',
            'preview'   => '123',
        ),
        'woocommerce_order_status' => array(
            'label'     => __( 'Order Status', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => __( 'Completed', 'automatorwp-woocommerce' ),
        ),
        'woocommerce_order_date_created' => array(
            'label'     => __( 'Order Date Created', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => date( 'Y-m-d H:i:s' ),
        ),
        'woocommerce_order_date_paid' => array(
            'label'     => __( 'Order Date Paid', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => date( 'Y-m-d H:i:s' ),
        ),
        'woocommerce_order_customer_ip_address' => array(
            'label'     => __( 'Order Customer IP Address', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => '255.255.255.255',
        ),
        'woocommerce_order_customer_note' => array(
            'label'     => __( 'Order Customer Note', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => __( 'The customer note entered at checkout', 'automatorwp-woocommerce' ),
        ),
        'woocommerce_order_coupon_codes' => array(
            'label'     => __( 'Order Coupon Codes', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => 'CODE01, CODE02',
        ),
        'woocommerce_order_currency' => array(
            'label'     => __( 'Order Currency', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => 'USD',
        ),
        'woocommerce_order_currency_symbol' => array(
            'label'     => __( 'Order Currency Symbol', 'automatorwp-woocommerce' ),
            'type'      => 'text',
            'preview'   => '$',
        ),
        'woocommerce_order_subtotal' => array(
            'label'     => __( 'Order Subtotal', 'automatorwp-woocommerce' ),
            'type'      => 'float',
            'preview'   => '123.45',
        ),
        'woocommerce_order_discount_total' => array(
            'label'     => __( 'Order Discount', 'automatorwp-woocommerce' ),
            'type'      => 'float',
            'preview'   => '123.45',
        ),
        'woocommerce_order_total_tax' => array(
            'label'     => __( 'Order Tax', 'automatorwp-woocommerce' ),
            'type'      => 'float',
            'preview'   => '123.45',
        ),
        'woocommerce_order_total' => array(
            'label'     => __( 'Order Total', 'automatorwp-woocommerce' ),
            'type'      => 'float',
            'preview'   => '123.45',
        ),
    ...
    foreach( automatorwp_woocommerce_get_address_fields() as $field ) {
        ...
        $order_tags['woocommerce_order_billing_' . $field] = array(
            'label'     => sprintf( __( 'Billing %s', 'automatorwp-woocommerce'  ), automatorwp_woocommerce_get_address_field_label( $field ) ),
            'type'      => 'text',
            'preview'   => automatorwp_woocommerce_get_address_field_preview( $field ),
        );
    ...
    foreach( automatorwp_woocommerce_get_address_fields() as $field ) {
        ...
        $order_tags['woocommerce_order_shipping_' . $field] = array(
            'label'     => sprintf( __( 'Shipping %s', 'automatorwp-woocommerce'  ), automatorwp_woocommerce_get_address_field_label( $field ) ),
            'type'      => 'text',
            'preview'   => automatorwp_woocommerce_get_address_field_preview( $field ),
        );
...
/**
 * Custom action tag replacement for 'Get Order Details' action
 *
 * @since 2.0.0
 *
 * @param string    $replacement    The tag replacement
 ...
 * @return string
 */
function automatorwp_woocommerce_get_order_details_tag_replacement( $replacement, $tag_name, $action, $user_id, $content, $log ) {

    // Bail if not our action
    if( $action->type !== 'woocommerce_get_order_details' ) {
        return $replacement;
    }

    // Bail if tag does not match our action prefix
    if( substr( $tag_name, 0, 20 ) !== 'woocommerce_action_' ) {
        return $replacement;
    }

    // Get the order ID that we stored in the listener
    $order_id = (int) automatorwp_get_log_meta( $log->id, 'order_id', true );

    // Bail if no order ID is available
    if( $order_id === 0 ) {
        return $replacement;
    }

    // Specific tag replacement for the Order ID used in the action
    if( $tag_name === 'woocommerce_action_order_id' ) {
        return $order_id;
    }

    // Map the action tag name to the generic order tag name (e.g., woocommerce_action_order_total -> woocommerce_order_total)
    $order_trigger_tag = str_replace( 'woocommerce_action_', 'woocommerce_', $tag_name );
    
    // Use the existing trigger tag replacement logic to resolve the order details.
    // We pass the order_trigger_tag and other arguments to reuse existing code in the file.
    $replacement = automatorwp_woocommerce_get_trigger_tag_replacement( $replacement, $order_trigger_tag, $action, $user_id, $content, $log );

    return $replacement;

}
add_filter( 'automatorwp_get_action_tag_replacement', 'automatorwp_woocommerce_get_order_details_tag_replacement', 10, 6 );

/**
 * Register order details action tags
 *
 * @since 2.0.0
 *
 * @param array $tags The tags
 * @return array
 */
function automatorwp_woocommerce_get_order_details_action_tags( $tags ) {

    // Asegúrate de que esta función automatorwp_woocommerce_get_order_details_tags()
    // está definida antes en este archivo.
    if( function_exists( 'automatorwp_woocommerce_get_order_details_tags' ) ) {
        $tags['woocommerce_get_order_details'] = array(
            'label' => __( 'WooCommerce - Get Order Details', 'automatorwp-woocommerce' ),
            'tags'  => automatorwp_woocommerce_get_order_details_tags(),
            'icon'  => AUTOMATORWP_WOOCOMMERCE_URL . 'assets/woocommerce.svg',
        );
    }
...
add_filter( 'automatorwp_get_action_tags', 'automatorwp_woocommerce_get_order_details_action_tags' );