<?php
/**
 * Purchase Product Multiple
 *
 * @package     AutomatorWP\Integrations\WooCommerce\Triggers\Purchase_Product_Multiple
...
class AutomatorWP_WooCommerce_Purchase_Product_Multiple extends AutomatorWP_Integration_Trigger {

    public $integration = 'woocommerce';
    public $trigger = 'woocommerce_purchase_product_multiple';
    ...
    public function register() {
        ...
        automatorwp_register_trigger( $this->trigger, array(
            ...
            'label'             => __( 'User purchases specific products', 'automatorwp-woocommerce' ),
            'select_option'     => __( 'User purchases specific <strong>products</strong>', 'automatorwp-woocommerce' ),
            /* translators: %1$s: Post title. %2$s: Number of times. */
            'edit_label'        => sprintf( __( 'User purchases specific %1$s %2$s time(s)', 'automatorwp-woocommerce' ), '{products}', '{times}' ),
            /* translators: %1$s: Post title. */
            'log_label'         => sprintf( __( 'User purchases specific %1$s', 'automatorwp-woocommerce' ), '{products}' ),
            ...
            'options'           => array(
                'products' => array(
                    ...
                    'fields' => array(
                        'contain' => array(
                            'name' => __( 'Order contains products:', 'automatorwp' ),
                            'desc' => __( 'Check to see if the order contains the specified products. If not, only the specified products will be taken into account.', 'automatorwp' ),
                            'type' => 'checkbox',
                            'classes' => 'cmb2-switch'
                        ),
                        'products' => array(
    ...
    public function user_deserves_trigger( $deserves_trigger, $trigger, $user_id, $event, $trigger_options, $automation ) {
        ...
        if( (bool) $trigger_options['contain'] ) {
            
            // Option contain products enabled
            foreach ( $products_required as $product_required ) {
                if ( !in_array( $product_required, $products_order ) ) {
                    return false;
                }
            }
            ...
            // Order with specific products only
...
new AutomatorWP_WooCommerce_Purchase_Product_Multiple();