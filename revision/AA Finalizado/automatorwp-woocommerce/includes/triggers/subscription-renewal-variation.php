<?php
/**
 * Subscription Renewal Variation
 *
 * @package     AutomatorWP\Integrations\WooCommerce\Triggers\Subscription_Renewal_Variation
...
class AutomatorWP_WooCommerce_Subscription_Renewal_Variation extends AutomatorWP_Integration_Trigger {

    public $integration = 'woocommerce';
    public $trigger = 'woocommerce_subscription_renewal_variation';
    ...
    public function register() {
        ...
        automatorwp_register_trigger( $this->trigger, array(
            ...
            'label'             => __( 'User renews subscription of a product variation', 'automatorwp-woocommerce' ),
            'select_option'     => __( 'User <strong>renews</strong> subscription of a product variation', 'automatorwp-woocommerce' ),
            /* translators: %1$s: Product. %2$s: Variation. %3$s: Number of times. */
            'edit_label'        => sprintf( __( 'User renews subscription of %1$s with %2$s selected %3$s time(s)', 'automatorwp-woocommerce' ), '{product}', '{variation}', '{times}' ),
            /* translators: %1$s: Product. %2$s: Variation. */
            'log_label'         => sprintf( __( 'User renews subscription of %1$s with %2$s selected', 'automatorwp-woocommerce' ), '{product}', '{variation}' ),
            'action'            => 'woocommerce_renewal_order_payment_complete',
    ...
    public function listener( $order_id ) {
        ...
        foreach ($subscriptions as $subscriptionID => $subscriptionObj) {
            ...
            $subscription_id = $subscriptionObj->get_id();
        ...
        foreach ( $items as $item ) {
            ...
            for ( $i = 0; $i < $quantity; $i++ ) {
                ...
                // Trigger the product subscription renewal
...
new AutomatorWP_WooCommerce_Subscription_Renewal_Variation();